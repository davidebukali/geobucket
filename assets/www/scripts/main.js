...
$("#start").bind(
    "click",
    function(event, ui) {
>>>>>>> 2959dfa2c0cbbe5bdcdad276fc8743151a21d05c
...
<<<<<<< HEAD
      if((states[networkState] == 'No network connection') ||
      ...
      else
        {
          ...
          $ .blockUI({ message : '<h4><img
            src="images/ajax-loader.gif" /><br/>Checking
            Connection...</h4>', css : { top : ($(window).height()) /
                                          ...
                                          localStorage.anonymous != undefined) {
                                            upload(); } else {
                                              ...
                                              $('<div>') .simpledialog2( { mode : 'button', headerText :
                                                'Info...', headerClose : true, buttonPrompt : "You are
                                                  not logged in. Do you want to upload annonymously?",
          ...
          }).fail(function() { $.unblockUI(); alert("GeoBucket Site
              is Not Available")
              ...
              createTags(batchSize).then(function(tags, id, rowNumb)
                    {
                      ...
                      if(rowNumb > batchSize){
                        alert("rows: "+rowNumb+" greater than batch: "+batch); 
                        alert("The tags are: " + line);
                        ...
                        alert("rows: "+rowNumb+" < batch: "+batch+" id > 1: "+tempId); 
                      ...
                      /*
                       * sendData(parseInt(data[1], 10),
                       * data[0], username).then(function() {
                       * countUpload();
                       * 
                       ...
                       * upload(); countUpload();
                       * countDB(); // //alert("inside success //
                       * TempId is: " + tempId);
                       * 
                       * }).fail(function() { tempId = tempId -
                       * 1; upload(); countUpload();
                       * countDB(); // //alert("inside fail //
                       * TempId is: " + tempId); }); } else {
          ...
          function sendData(id, tags)
            {
              
              var d = $.Deferred();
              var title = "GeoBucket";
              // BEGIN: drupal services node create login (warning: don't use https if
              // you
          ...
          function onError(error)
            {
              ...
              if(error.message == gpsoff)
              ...
              else if(error.code == error.PERMISSION_DENIED && track != false)
                {
                  ...
                  window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, createLine,
                      failLineCreate);
                  // alert("Error PERMISSION_DENIED: "+error.message+",
                  // "+error.code);
              ...
              db.transaction(function(transaction)
                    {
                      transaction.executeSql(
                          ...
                          [ 0, tempId ], function(transaction, result)
                          {
                            ...
                            allTags = "(" + allTags + ")"; 
                            // console.log("line string count is:
                            // "+rows);
          ...
          function readPassAsText(file)
            {
              ...
              reader.onload = function(evt)
              {
                ...
                // alert("Read Pass and user: " + words[0] + " and " +
                // words[1]);
              };
          ...
          function readAsText(file)
            {
              ...
              reader.onload = function(evt)
              {
                ...
                // alert("Read accuracy and batch size: " + words[0] + " and
                // " +
                // words[1]);
              };
              reader.readAsText(file);
